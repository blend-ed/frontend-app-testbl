...
import ChatBoxDrawer from "../../../components/dashboard/common/chat/chat-drawer/ChatBoxDrawer";
import RecentChat from "../../../components/dashboard/common/chat/chat-drawer/RecentChat";
...
const GET_USERS = gql`
    subscription GetUsers($_eq: String) {
        user(where: {organisations: {sub_org: {name: {_eq: $_eq}}}}) {
            id
            created_at
            updated_at
            user_details(where: {user: {organisations: {sub_org: {name: {_eq: $_eq}}}}}) {
                name
                profile_image
            }
        }
...
const ChatDrawer = ({ show, onHide, setDrawerActive, drawerActive }) => {
    ...
    const { user } = useAuth0();
    const isStudent = !location.pathname.includes('/a/') && !location.pathname.includes('/i/') && !location.pathname.includes('/m');
    ...
    const { data: messageData } = useSubscription(GET_MESSAGES_USER, {
        variables: {
            _eq: user?.['https://hasura.io/jwt/claims']['x-hasura-user-id'],
            sub_org: sub_org_name
        }
    ...
    const { data: threadDataUser } = useSubscription(CHAT_THREAD_USER, {
        variables: {
            _eq: user?.['https://hasura.io/jwt/claims']['x-hasura-user-id'],
            sub_org: sub_org_name
        }
    ...
    useEffect(() => {
        if (usersData && instructorData) {
            ...
            isStudent && setUsersList((usersList) => [
                ...usersList,
                {
                    id: 'ai',
        ...
        if (messageData && aiData) {
            ...
            // Concatenate user
            const allMessages = [...userChatMessages];
        ...
        const combinedThreadData = [...(threadDataUser?.messages || [])];
        if (combinedThreadData) {
            ...
            if (aiDataThread && aiDataThread.ai_chat.length > 0 && isStudent) {
            ...
            } else if (aiDataThread && isStudent) {
                setFormattedThreadData((formattedThreadData) => [
                    {
                        id: 'ai',
                        userId: 'ai',
    ...
    }, [usersData, messageData, threadDataUser, instructorData, aiData, aiDataThread, groupChatroomMembers, chatroomMembers, allChatrooms]);

    return (
        <Offcanvas show={show} onHide={onHide} scroll={true} backdrop={false} placement='bottom' className={`${drawerActive && 'active'} me-4 chat-drawer rounded-top shadow-lg border`}>
            <Offcanvas.Header className='p-2 px-3 border-bottom' onClick={() => !chatBoxId && setDrawerActive(!drawerActive)}>
                {chatBoxId ?
                    <Offcanvas.Title onClick={() => setChatBoxId(null)} className="text-truncate-line">
                        <span className={`fe fe-chevron-left fw-bold me-1`} />
                        {activeThread?.type === 'user' ? usersList?.find(({ id }) => id === activeThread?.userId)?.name : 'Group'}
                    </Offcanvas.Title>
                    :
                    <Offcanvas.Title>
                        Chat
                ...
                <div className="d-flex">
                    <button className='btn btn-sm border-0' onClick={() => setDrawerActive(!drawerActive)}>
                        <span className={`fe fe-chevron-${drawerActive ? 'down' : 'up'} fw-bold fs-4`} />
                    </button>
                    <button className='btn btn-sm border-0 pe-0' onClick={onHide}>
                        <span className={`fe fe-x fw-bold fs-4`} />
                    </button>
            ...
            <Offcanvas.Body className="p-0">
                {chatBoxId ?
                    <ChatBoxDrawer hideChatBox={hideChatBox} setActiveThread={setActiveThread} formattedThreadData={formattedThreadData} chatMessageArray={chatMessageArray} usersList={usersList} groupChatList={groupChatList} setHideChatBox={setHideChatBox} chatBoxId={chatBoxId} aiChatName={aiChatName?.organisation[0]?.ai_chat} />
                    :
                    <RecentChat setChatBoxId={setChatBoxId} hideChatBox={hideChatBox} setHideChatBox={setHideChatBox} chatMessageArray={chatMessageArray} usersList={usersList} groupChatList={groupChatList} formattedThreadData={formattedThreadData} activeThread={activeThread} />
                }
            </Offcanvas.Body>
        </Offcanvas>
    )
